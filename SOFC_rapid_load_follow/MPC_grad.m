function [states_plant, input_plant, last_config, P_plant_hist, U_plant_hist, FU_plant_hist, Lair_plant_hist, eff_plant_hist, modif,duration_grad] = MPC_grad(optimal_var,output_nompla,modif,Pel_target,Ucell_target,T_0,T_in,SOFC_data_nominal,SOFC_data_plant,M,m,lb,ub,initial_config,last_plant_output)
Ts = 10;  % Time sample
...
for i = 1:3
    ...
    states_plant    = [states_plant, x_plant(:,1:end-1)];
    last_config     = [u_plant(:,end); x_plant(:,end-1)];
    duration_grad   = [duration_grad, size(u_plant,2)+duration_grad(i)];
end
