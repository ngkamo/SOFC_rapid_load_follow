function [A,B,C,D] = MPC_linearization_mat(u_ss,dx_ss,Pel_ss,U_ss,eff_ss,T_in,SOFC_data)
...
for i = 1:length(deltaCPel)
    ...
    u_grad = u_ss;
    u_grad(3+i) = u_grad(3+i) + deltaCPel(i);
    [dx_test,Ucell_test,Pel_test,FU_test,Lair_test,~,eff_test] = fPrimeMyProject(0,u_grad(4:end),u_grad(1:3),T_in,SOFC_data);
    C(3,i) = (eff_test-eff_ss)/deltaCEff(i);
end
...
for i = 1:length(deltaDPel)
    ...
    u_grad(i) = u_grad(i) + deltaDUc(i);
    [dx_test,Ucell_test,Pel_test,FU_test,Lair_test,~,eff_test] = fPrimeMyProject(0,u_grad(4:end),u_grad(1:3),T_in,SOFC_data);
    D(3,i) = (eff_test-eff_ss)/deltaDEff(i);
end
